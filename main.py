...
async def start_handler(msg: Message):
    ...
    if not chat_members:
        await msg.answer('Ошибка при получении участников чата.')
        return

    await add_members_to_database(chat_id, [member.id for member in chat_members], points=0)
...
async def reset_points(chat_id: int):
    try:
...
async def drop_table(chat_id: int):
    try:
        table_name = f"chat_{chat_id}"
...
async def reset_and_recreate_table(chat_id: int):
        ...
        async with aiosqlite.connect('chat_members.db') as conn:
            async with conn.cursor() as cursor:
                for member_id in member_ids:
                    await cursor.execute(f'''
                        INSERT INTO {table_name} (member_id, points)
                        VALUES (?, ?)
                    ''', (member_id, points))
...
async def add_toxic_words(chat_id: int, member_id: int, toxic_word: str):
    try:
        table_name = f"chat_{chat_id}"
        ...
        async with aiosqlite.connect('chat_members.db') as conn:
            async with conn.cursor() as cursor:
                ...
                if existing_record:
                    existing_toxic_words = existing_record[1]  # Получаем текущие токсичные слова
                    ...
                    await cursor.execute(f'''
                            ...
                            SET toxic_words = ?
                            WHERE member_id = ?
                        ''', (new_toxic_words, member_id))
                else:
                    # Если записи нет, просто добавляем новую запись
                    await cursor.execute('''
                            INSERT INTO {table_name}(member_id, toxic_words)
                            VALUES (?, ?)
                        ''', (member_id, toxic_word))
...
async def get_member_toxic_words(chat_id: int, member_id: int):
    try:
        table_name = f"chat_{chat_id}"

        async with aiosqlite.connect('chat_members.db') as conn:
            async with conn.cursor() as cursor:
                # Проверяем, существует ли уже запись с указанным member_id
                await cursor.execute(f'''
                        SELECT toxic_words
                        FROM {table_name}
                        WHERE member_id = ?
                ...
                if result:
                    toxic_words_str = result[0]  # Получаем токсичные слова пользователя
                    if toxic_words_str:
                        toxic_words = toxic_words_str.split(', ')
                    else:
                        toxic_words = []
                else:
                    toxic_words = []  # Если пользователя с указанными member_id нет, возвращаем пустой список

        return toxic_words

    except Exception as e:
        print(f"Error get toxic word: {e}")


@router.message(Command('words'))
async def on_get_toxic_words(msg: Message):
    if msg.reply_to_message:
        try:
            chat_id = msg.chat.id
            member_id = msg.reply_to_message.from_user.id
            ...
            toxic_words = await get_member_toxic_words(chat_id, member_id)

            if toxic_words:
                toxic_words_str = ', '.join(toxic_words)
                await msg.answer(f"Токсичные слова пользователя {username}: {toxic_words_str}")
            else:
                await msg.answer(f"Пользователь {username} не использовал токсичные слова!")

        except Exception as e:
            print(f"Error processing /words command: {e}")
    else:
        await msg.answer('Если ты хочешь узнать список слов токсика, ответь на его сообщение)')


@router.message(Command('points'))
async def show_member_points(msg: Message):
    try:
        # Получаем chat_id и ID пользователя, на чье сообщение отвечаем
        chat_id = msg.chat.id
        ...
        async with aiosqlite.connect('chat_members.db') as conn:
            async with conn.cursor() as cursor:
                ...
                await cursor.execute(f'SELECT points FROM {table_name} WHERE member_id = ?', (replied_user_id,))
...
async def calculate_toxicity(member_id, user_points, all_member_points):
    total_points = sum(points for points in all_member_points.values())
    toxicity_percent = (user_points / total_points) * 100
    return toxicity_percent


@router.message(Command('toxicity_stats'))
async def toxicity_stats_command(msg: Message):
    replied_user_id = msg.reply_to_message.from_user.id
    chat_id = msg.chat.id  # Получаем chat_id текущего чата

    all_member_points = await get_all_member_points(chat_id)  # Передаем chat_id в функцию

    if all_member_points:
        user_points = all_member_points.get(replied_user_id)
