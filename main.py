...
async def start_handler(msg: Message):
    chat_id = msg.chat.id
    already_synced = await check_and_sync_chat_members(chat_id)

    if already_synced:
        await bot.send_message(chat_id, "Команда старт уже была запущена.")
        return
    ...
    if not already_synced:
        try:
            ...
            await msg.answer('Не забудь назначить меня администратором, это нужно для работы некоторых моих функций',
...
async def check_and_sync_chat_members(chat_id: int):
    ...
    try:
        async with aiosqlite.connect('chat_members.db') as conn:
            async with conn.cursor() as cursor:
                ...
                if not table_exists:
                    return False  # Таблица не существует, ничего не делаем

                chat_members = await get_chat_members(chat_id)

                await cursor.execute(f"SELECT member_id FROM {table_name}")
                table_members = await cursor.fetchall()
                table_members_ids = [row[0] for row in table_members]

                for member_id in chat_members:
                    if member_id not in table_members_ids:
                        await cursor.execute(f"INSERT INTO {table_name} (chat_id, member_id, points, toxic_words) VALUES (?, ?, 0, '')", (chat_id, member_id))
                        await conn.commit()

                for member_id in table_members_ids:
                    if member_id not in chat_members:
                        await cursor.execute(f"DELETE FROM {table_name} WHERE member_id = ?", (member_id,))
                        await conn.commit()

                if set(chat_members) == set(table_members_ids):
                    return True  # Все участники уже синхронизированы

                return False  # Есть изменения
    ...
    async with aiosqlite.connect('chat_members.db') as conn:
        async with conn.cursor() as cursor:
            ...
            if points > 0:
                ...
                ''', (member_id, chat_id))
                await conn.commit()
                points_word = get_points_word(points-1)
                await msg.answer(
                    f"Слово '{toxic_word}' удалено у пользователя {username}. Теперь у него {points - 1} {points_word}")
            else:
            ...
            async with conn.cursor() as cursor:
                ...
                if row:
                    ...
                    if points > 0:
                        ...
                        response = random.choice(more_points).format(username=username, points=points, points_word=points_word)
                    else:
